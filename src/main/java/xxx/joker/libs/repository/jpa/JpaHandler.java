package xxx.joker.libs.repository.jpa;
...
public class JpaHandler {
    ...
    private Map<Class<?>, ProxyDataSet> proxies;
    ...
    public JpaHandler(RepoCtx ctx, List<RepoEntity> entities) {
        ...
        this.proxies = JkStreams.toMapSingle(ctx.getClazzWraps().keySet(), k -> k, k -> new ProxyDataSet());
    ...
    private boolean addEntityToRepo(RepoEntity e) {
        ...
        fwraps.forEach(fw -> {
            Object fvalue = fw.getValue(e);
            if(fw.isEntityFlat()) {
                if(fw.isEntity()) {
                    if(fvalue != null) {
                        RepoEntity edep = (RepoEntity) fvalue;
                    ...
                    List<RepoEntity> depList = (List<RepoEntity>) fvalue;
                    ...
                    Set<RepoEntity> depSet = (Set<RepoEntity>) fvalue;
            ...
            entList.forEach(re -> {
                fwList.forEach(fw -> {
                    if(fw.isEntity()) {
                        ...
                        coll.removeIf(ecoll -> ecoll.equals(re));
                    }
                });
            });
        });
