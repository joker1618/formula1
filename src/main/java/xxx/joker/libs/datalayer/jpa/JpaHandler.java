...
public class JpaHandler {
    ...
    private Map<Long, Map<Long, Set<FieldWrap>>> usedRefMap1;
    private Map<Long, TreeSet<Long>> usedRefMap;
                ...
                if(!entities.isEmpty()) {
                    ...
                    emap.forEach((ec, elist) -> {
                        ...
                        elist.forEach(v -> {
                            ...
                            dataById.put(eid, v);
                            usedRefMap.put(eid, new TreeSet<>());
                            usedRefMap1.put(eid, new TreeMap<>());
...
//                            usedRefMap1.put(eid, new TreeMap<>());
                            fwList.forEach(fw -> {
                                if(fw.isCollection()) {
                                    ...
                                    coll.forEach(re -> usedRefMap.get(re.getEntityId()).add(eid));
                                    coll.forEach(re -> {
                                        Long refId = re.getEntityId();
                                        usedRefMap1.putIfAbsent(refId, new HashMap<>());
                                        usedRefMap1.get(refId).putIfAbsent(eid, new HashSet<>());
                                        usedRefMap1.get(refId).get(eid).add(fw);
                                    });
                                } else {
                                    RepoEntity re = fw.getValueCast(v);
                                    if(re != null) {
                                        usedRefMap.get(re.getEntityId()).add(eid);
                                        Long refId = re.getEntityId();
                                        usedRefMap1.putIfAbsent(refId, new HashMap<>());
                                        usedRefMap1.get(refId).get(eid).add(fw);
                                    }
        ...
        try {
            ...
            synchronized (idSeqValue) {
                ...
                if (add) {
                    ...
                    usedRefMap1.putIfAbsent(eid, new HashMap<>());
                    refs.forEach((id, fwList) -> {
                        usedRefMap1.putIfAbsent(id, new HashMap<>());
                        usedRefMap1.get(id).putIfAbsent(eid, new HashSet<>());
                        usedRefMap1.get(id).get(eid).addAll(fwList);
                    });
                    updatePropertyIdSeq();
...
//            });
            ...
            usedRefMap1.get(eid).forEach((parId, fwList) -> {
