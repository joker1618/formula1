...
public class Year1984 extends AWikiParser2018 {
...
//                F1Entrant e = new F1Entrant();
                ...
                if(chTag == null) {
                    if(year == 1984 && tr.getTextFlat().contains("FA1E")) {
                        continue;
                    }
                }
                if(chTag.getAttribute("class").equals("nowrap")) {
...
//                F1Entrant e = new F1Entrant();
                ...
                F1Driver d = retrieveDriver(aTag.getText(), true);
//                F1Driver d = retrieveDriver(aTag.getAttribute("title"), true);
                if(StringUtils.isBlank(d.getCountry())) {
                    JkTag img = spanTag.findFirstTag("img");
                    d.setCountry(fixNation(img.getAttribute("alt")));
...
//                F1Entrant e = new F1Entrant();
            ...
            if(e != null && year==1985 && e.getDriver().getFullName().equals("Stefan Bellof")) {
                ...
                model.add(e2);
            } else if(e != null && year==1985 && e.getDriver().getFullName().equals("Kenny Acheson")) {
                F1Entrant e2 = new F1Entrant();
                e2.setYear(year);
                e2.setTeam(e.getTeam());
                e2.setCarNo(9);
...
//        System.exit(1);
    ...
    protected Map<String, Double> getExpectedDriverPoints(String html) {
        ...
        JkTag tableEntrants = JkScanners.parseHtmlTag(html, "table", "<span class=\"mw-headline\" id=\"Formula_1_World_Championship_for_Drivers_–_final_standings\">"
    ...
    protected Map<String, Double> getExpectedTeamPoints(String html) {
        ...
        JkTag tableEntrants = JkScanners.parseHtmlTag(html, "table", "<span class=\"mw-headline\" id=\"Formula_1_World_Championship_for_Manufacturers_–_final_standings\">",
                "<table");
        JkTag tbody = tableEntrants.getChild("tbody");

        for (JkTag tr : tbody.getChildren("tr")) {
            if((tr.getChildren("th").size() == 1 && tr.getChildren().get(0).getTagName().equals("th"))
                || (year == 1984 && tr.getChildren("th").size() == 0 && tr.getChildren("td").size() == 20)) {
