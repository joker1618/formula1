...
public class Year2006 extends AWikiParser2018 {
...
//        System.exit(1);
    ...
    protected void parseQualify(String html, F1GranPrix gp) {
        ...
        Map<Integer, Integer> qualRoundPos = new HashMap<>();
        int pos = 1;
        ...
        for (JkTag tr : tbody.getChildren("tr")) {
            ...
            if(tdNum == 0 && thNum == tr.getChildren().size() && qualRoundPos.isEmpty()) {
                for (int i = 0; i < thNum; i++) {
                    String htxt = tr.getChild(i).getText();
                    if("Q1".equals(htxt))   qualRoundPos.put(i, 1);
                    if("Q2".equals(htxt))   qualRoundPos.put(i, 2);
                    if("Q3".equals(htxt))   qualRoundPos.put(i, 3);
                }

            } else if(thNum == 1 && (tdNum >= 6 && tdNum <= 8)) {
                ...
                Map<Integer, JkDuration> timesMap = new HashMap<>();
                for (Integer index : qualRoundPos.keySet()) {
                    timesMap.put(qualRoundPos.get(index), getQualTime(tr.getChild(index)));
                }
                for(int i = 1; i <= 3; i++)  q.getTimes().add(timesMap.get(i));
