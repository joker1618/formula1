...
import xxx.joker.libs.repository.design.EntityCreationTm;
import xxx.joker.libs.repository.design.EntityID;
...
import static xxx.joker.libs.repository.config.RepoConfig.CsvSeparator.SEP_FIELD;
...
public class ClazzWrap {
    ...
    public RepoEntity parseEntity(Map<String, String> strValues) {
        ...
        for(String mapFName : strValues.keySet()) {
            FieldWrap cf = fieldByNameMap.get(mapFName);
            if(cf != null) {
    ...
    public List<Pair<String,String>> formatEntity(RepoEntity e) {
        ...
        fieldByNameMap.forEach((fname, cf) -> toRet.add(Pair.of(fname, cf.formatValue(e))));
        return toRet;
    }
